...
Text-to-image diffusion model의 성능을 끌어올리기 위해서 여러 방법들이 연구되었다. (여러 방법들이 있으나 무슨 의미인지 바로 알 수가 없다...)
...
Conditional diffusion probabilistic models에서는 $p(x_t|c)$, 즉 context $c$가 주어졌을 때 샘플 데이터 $x_0$를 추정하게 된다. 구체적으로는 Markovian forward process의 역과정 $p(x_t|x_{t-1})$을 여러 번 거치면서 noise를 추가하게 되는 것이다. 아울러 이 과정을 모사하는 network $\mu_\theta(x_t,c,t)$도 학습하게 된다.
...
$x_T\sim\mathcal{N}(0,\mathbf{I})$를 샘플링 한 뒤 reverse process $p_\theta(x_{t-1}|x_t,c)$를 반복하여 최종적으로는 $x_0$를 얻게 된다.
...
P(s_{t+1} \mid s_t, a_t) \triangleq (\delta_c, \delta_{t-1}, \delta_{x_{t-1}}), \quad 
...
<center><img src='{{"assets/images/DDPO/ddpo1.png" | relative_url}}' width="100%"></center>

각 reward function을 통해 학습한 뒤의 정성적 결과

<center><img src='{{"assets/images/DDPO/ddpo3.png" | relative_url}}' width="100%"></center>

학습 진행에 따른 각 task에서의 결과물

### Generalization

<center><img src='{{"assets/images/DDPO/ddpo5.png" | relative_url}}' width="100%"></center>
...
<center><img src='{{"assets/images/DDPO/ddpo6.png" | relative_url}}' width="100%"></center>
